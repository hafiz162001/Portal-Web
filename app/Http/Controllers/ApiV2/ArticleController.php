...
class ArticleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            $articles = Article::query()->with('galleries', 'article_details')->withCount([
                'views',
                'is_liked' => function($q) use($category) {
                    $q
                    ->where('likes.type', '=', 'article')->where('likes.category', '=', $category);
                },
                'liked' => function($q) use($category) {
                    $q
                    ->where('likes.type', '=', 'article')->where('likes.category', '=', $category);
                },
                'comment' => function($q) use($category) {
                    $q
                    ->where('comments.type', '=', 'article')->where('comments.category', '=', $category);
                },
            ]);
